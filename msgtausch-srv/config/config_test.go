...
func TestLoadConfigJSON_InterceptionConfig(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			configPath := createTempConfigFile(t, t.TempDir(), "config.json", tc.configJSON)
			cfg, err := LoadConfig(configPath)
...
`,
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			configPath := createTempConfigFile(t, t.TempDir(), "config.hcl", tc.configHCL)
			cfg, err := LoadConfig(configPath)
...
func TestLoadConfig_InterceptionConfigPrecedence(t *testing.T) {
	...
	configPath := createTempConfigFile(t, tempDir, "config.json", configJSON)
...
interception = {
	...
	configPath := createTempConfigFile(t, t.TempDir(), "config.hcl", hclConfig)
	cfg, err := LoadConfig(configPath)
	if err != nil {
...
func TestHTTPSClassifierConfigEnv(t *testing.T) {
	...
	configPath := createTempConfigFile(t, t.TempDir(), "config.json", configJSON)
	cfg, err := LoadConfig(configPath)
	if err != nil {
		t.Fatalf("Failed to load config: %v", err)
	}
