...
target/debug/msgtausch-simulation --scenario crates/msgtausch-simulation/corpus/http-upgrades-794.json --enable-forwards --stats
...
target/debug/msgtausch-simulation --tls-interception --binary target/debug/msgtausch
```
...
The TLS interception simulation creates a TLS origin, starts the compiled proxy
with the checked-in test CA, then sends CONNECT followed by a TLS request that
trusts that CA. It checks the issued leaf, downstream handshake, upstream TLS
connection, and request forwarding. The origin uses a separate self-signed
certificate, so this also checks the configured `insecure-skip-verify` path.

HTTP/3, TCP and DNS-over-TLS resolvers, reload, and domain-list refresh or
failure are not in this runner yet. Those paths need protocol-specific fixtures
and should not be faked with unit-level calls.
