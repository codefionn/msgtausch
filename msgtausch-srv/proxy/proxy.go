...
func (p *Proxy) CompileClassifiers() {
	...
	if p.config.Forwards != nil {
		for _, fwd := range p.config.Forwards {
			cf, err := CompileClassifier(fwd.Classifier())
			if err != nil {
				logger.Error("Error compiling classifier for forward: %v", err)
				continue
			}
...
func NewProxy(cfg *config.Config) *Proxy {
	...
	for _, serverCfg := range cfg.Servers {
		...
		case config.ProxyTypeHTTPS:
			...
			if err != nil {
				logger.Error("Failed to read CA certificate file '%s': %v", caFile, err)
				continue
			}

			caKey, err := os.ReadFile(caKeyFile)
			if err != nil {
				logger.Error("Failed to read CA key file '%s': %v", caKeyFile, err)
				continue
			}
...
func writeProxyErrorResponse(w http.ResponseWriter, originalErr error, defaultErrorCode string) {
	...
	if badGatewayResp.Body != nil {
		...
		badGatewayResp.Body.Close() // Ensure the NopCloser's underlying reader is "closed" (though for bytes.Reader it's a no-op)
	}
}
